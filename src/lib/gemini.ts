...
const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" }); // confirmed available
...
export interface MessageAnalysis {
  ...
  is_general_question: boolean; // true = FAQ/info query, not a service request
  has_problem: boolean;
...
const TRADE_KEYWORDS: Record<string, string[]> = {
  HVAC:        ["ac", "a/c", "air condition", "hvac", "cooling", "heat", "heater", "heating", "geyser", "geysir", "fan", "air cool", "leakage from ac", "ac not", "gas leakage"],
...
const FAQ_WORDS = ["what service", "what do you", "how does", "how do you", "what trades", "like what", "tell me about", "what kind", "what type", "kya karte", "kya services", "explain", "introduce", "about you", "what is this", "who are you", "what are you", "weekend", "timing", "hours", "coverage", "area serve"];

function keywordFallback(text: string): MessageAnalysis {
  const lower = text.toLowerCase();

  const is_general_question = FAQ_WORDS.some(k => lower.includes(k));
  ...
  let follow_up = "";
  if (is_general_question) follow_up = "";
  else if (!has_problem) follow_up = "Hi! Please describe your home service problem and share your city/area. Example: *My AC is leaking, Islamabad, Barakhu*";
  else if (!has_location) follow_up = "Got it! Now please share your city and area. Example: Islamabad, Barakhu";

  return { language: "english", is_general_question, has_problem, has_location, trade, summary: text.slice(0, 100), city, area, follow_up };
}
...
// ── Bid regex fallback (runs when Gemini fails) ───────────────────────────────

function regexExtractBid(text: string): BidExtraction {
  const lower = text.toLowerCase();

  let price = "Not specified";
  // Match: "2000", "rs 2000", "2,000 rupees", "price: 2000", "one thousand" handled by Gemini
  const priceMatch = lower.match(/(?:rs\.?\s*|pkr\s*|price\s*[:\s]+|rupees?\s*)?([\d,]{3,})\s*(?:rs|pkr|rupees?)?/i);
  if (priceMatch) {
    const num = priceMatch[1].replace(/,/g, "");
    if (parseInt(num) >= 100) price = `Rs. ${num}`;
  }

  let eta = "Not specified";
  // Match: "1 hour", "30 minutes", "30 min", "2 hrs", "45mins", "eta: 1 hour"
...
Return ONLY valid JSON with no markdown:
{
  ...
  "is_general_question": true/false — true if they are asking a general question (what services, how it works, timings, coverage, do you work weekends, etc.) and NOT describing a specific problem,
  "has_problem": true/false — did they describe a specific home service problem?,
  "has_location": true/false — did they mention a city or area in Pakistan?,
  ...
  "follow_up": "if is_general_question is true, leave this empty string. Otherwise ask for what is MISSING — if problem missing ask them to describe it, if location missing ask for city and area. Reply in the SAME LANGUAGE the customer used. Keep it warm and brief."
}

Important: 'is_general_question' and 'has_problem' are mutually exclusive. If someone asks 'do you fix ACs?' that is a general question (is_general_question: true), not a service request.`;

  try {
    const result = await model.generateContent(prompt);
    const raw = clean(result.response.text());
...
export async function extractBidDetails(text: string): Promise<BidExtraction> {
  ...
  const prompt = `A technician in Pakistan is submitting a bid for a home service job. Their message: "${text}"
...
- "one thousand pkr, i ll come in ten minutes" → {"price":"Rs. 1000","eta":"10 minutes"}
- "1500 30 minutes" → {"price":"Rs. 1500","eta":"30 minutes"}
- "rs2500" → {"price":"Rs. 2500","eta":"Not specified"}

If a field is truly not mentioned, write exactly "Not specified" for that field only.`;
  try {
    const result = await model.generateContent(prompt);
    const raw = clean(result.response.text());
